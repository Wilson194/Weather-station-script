...
import serial           # slouží pro komunikaci po COMnn portech
import time             # slouží k časovým prodlevám
import csv,string       # slouží pro export do csv formátu
import os.path




# načte data z seriové linky

def nacti_data():
    ser = serial.Serial()    # připojení na seriovou linku
    ser.bytesize = 8
    ser.port = nastaveni["com_port"]
    ser.bautrate = 19200
    ser.timeout = 5
    ser.open()
    ...
    vystup = ser.read(ser.inWaiting())   # přečtení dat
    ser.close()
...
def nacti_nastaveni():
    ...
    f = file("settings.csv","r")        #otevření souboru s nastavením
    data = csv.reader(f,dialect="excel",delimiter=';',)
    for nazev,hodnota in data:          # načtení hodnot do slovníku
        nastaveni[nazev] = hodnota
    f.close()

    global poradi
    s = file("poradi.csv","r")          # načtení souboru s pořadím pro ukládání
    poradi = s.readline()
    poradi = poradi.split(";")
    s.close()

def zpracuj_data(data):                 # rozdělí data po bitech na příslušné data
    a = data
...
def preved_jednotky(hodnoty):           # převede na potřebné jednotky
    h = hodnoty
...
def zobraz_data(data):          # vystup do souboru show.csv
    ...
    for i in poradi:                        #porovnání s nastavením, které hodnoty má uložit 
        if nastaveni[i] == "1":
            csvvystup.append(data[i])

    cesta = nastaveni["shows_file_path"] + "show.csv"
    if os.path.exists(cesta):               # načtení stávajících dat
        f = file(cesta,"r+")
        stare = f.readlines()
        f.close()
    else:
    ...
    f = file(cesta,"w+")
    for i in stare:                         # zapsání nových dat
        f.write(i)
...
def start():                                #spuštění potřebných funkcí
    data = nacti_data()
